...
def profile(request):
    ...
    for event in events:
        rating_avg = Rating.objects.filter(
            event_id=event.id).aggregate(Avg('rating'))
...
class EventDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        rating_avg = Rating.objects.filter(
            event_id=self.kwargs['pk']).aggregate(Avg('rating'))
...
class EventCreate(LoginRequiredMixin, CreateView):
    model = Event
    fields = ['event_name', 'address', 'description', 'date',
              'hours_of_op', 'covid_protocol', 'admission_fee', 'age_rating']
    success_url = '/events'
